...
- **MM-102** Backend deploy to AWS via Terraform — ECS Fargate (decided above), containers built by the existing CI pipeline (already pushes to Docker Hub), IAM role attached to `iac/iam.tf`'s existing least-privilege policy (also sitting unattached since MM-8).
- **MM-103** Frontend deploy to Vercel (free subdomain, decided above), wired to the deployed backend's real URL (not localhost).
