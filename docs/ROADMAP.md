...
- **Wire `rating_triggers` into breach evaluation.** Found (2026-08-05) that a rating downgrade is currently a no-op: `MarketEventType.DOWNGRADE` is a real event type with a working simulated scenario (`make simulate SCENARIO=downgrade`) that correctly routes through the Event Agent and triggers a real orchestrator run -- but `evaluate_breach()`/`CSATerms` only know `threshold`/`mta`/`currency`, so the run recomputes against the same unchanged price and threshold and will essentially always resolve "no breach". Meanwhile `csa_rag.py` already extracts `rating_triggers` from the actual CSA document text (the LLM is already asked "what are the rating triggers?" during term extraction) -- that data just gets discarded instead of reaching `CSATerms`. The fix: thread `rating_triggers` through to `CSATerms`/`evaluate_breach` so a real rating-trigger clause (e.g. "threshold drops to zero below BBB") actually changes the threshold used in the calc -- deterministic rule application (golden rule 1), not an LLM judgment call at evaluation time; the LLM's job stays just the one-time extraction of what the clause says. Out of scope for this story, flagged as a distinct, more speculative follow-on if wanted later: *proactive* downgrade-based risk flagging (using a downgrade as a leading indicator before any threshold math changes at all) -- a genuinely different feature, and one of the few places an LLM judgment call would legitimately belong, unlike the deterministic trigger-threshold fix itself.
